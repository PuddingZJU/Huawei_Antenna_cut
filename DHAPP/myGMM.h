...
{
public:
	myGMM(int dimNum = 1, int mixNum = 1);
	~myGMM();

	void Copy(myGMM* myGMM);
	...
	friend std::ostream& operator<<(std::ostream& out, myGMM& myGMM);
	friend std::istream& operator>>(std::istream& in, myGMM& myGMM);
