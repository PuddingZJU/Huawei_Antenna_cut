...
myGMM::myGMM(int dimNum, int mixNum)
{
	m_dimNum = dimNum;
	m_mixNum = mixNum;
...
void myGMM::Copy(myGMM* myGMM)
{
	assert(m_mixNum == myGMM->m_mixNum && m_dimNum == myGMM->m_dimNum);

	for (int i = 0; i < m_mixNum; i++)
	{
		m_priors[i] = myGMM->Prior(i);
		memcpy(m_means[i], myGMM->Mean(i), sizeof(double) * m_dimNum);
		memcpy(m_vars[i], myGMM->Variance(i), sizeof(double) * m_dimNum);
	}
	memcpy(m_minVars, myGMM->m_minVars, sizeof(double) * m_dimNum);
}

double myGMM::GetProbability(const double* sample)
{
	double p = 0;
	for (int i = 0; i < m_mixNum; i++)
	{
...
double myGMM::GetProbability(const double* x, int j)
{
	double p = 1;
	for (int d = 0; d < m_dimNum; d++)
	{
...
void myGMM::Train(const char* sampleFileName)
{
...
void myGMM::Train(double *data, int N)
{
	Init(data,N);
...
void myGMM::Init(double *data, int N)
{
	...
	for (int i = 0; i < m_mixNum; i++)
	{
		...
		else
		{
			...
			cout << "[WARNING] Gaussian " << i << " of myGMM is not used!\n";
		}
	}
	delete kmeans;
	delete[] x;
...
void myGMM::Init(const char* sampleFileName)
{
	const double MIN_VAR = 1E-10;

	KMeans* kmeans = new KMeans(m_dimNum, m_mixNum);
	kmeans->SetInitMode(KMeans::InitUniform);
	kmeans->Cluster(sampleFileName, "myGMM_init.tmp");
	...
	ifstream labelFile("myGMM_init.tmp", ios_base::binary);
	assert(labelFile);
...
void myGMM::DumpSampleFile(const char* fileName)
{
...
ostream& operator<<(ostream& out, myGMM& myGMM)
{
	out << "<myGMM>" << endl;
	out << "<DimNum> " << myGMM.m_dimNum << " </DimNum>" << endl;
	out << "<MixNum> " << myGMM.m_mixNum << " </MixNum>" << endl;

	out << "<Prior> ";
	for (int i = 0; i < myGMM.m_mixNum; i++)
	{
		out << myGMM.m_priors[i] << " ";
	}
	out << "</Prior>" << endl;
	...
	for (int i = 0; i < myGMM.m_mixNum; i++)
	{
		for (int d = 0; d < myGMM.m_dimNum; d++)
		{
			out << myGMM.m_means[i][d] << " ";
		}
		out << endl;
	}
	out << "</Mean>" << endl;
...
istream& operator>>(istream& in, myGMM& myGMM)
{
	char label[50];
	in >> label; // "<myGMM>"
	assert(strcmp(label, "<myGMM>") == 0);

	myGMM.Dispose();

	in >> label >> myGMM.m_dimNum >> label; // "<DimNum>"
	in >> label >> myGMM.m_mixNum >> label; // "<MixNum>"
